...
public class SoapClient {
    private static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(SoapClient.class);
...
//                String cpResponse = result.getSOAPPart().getTextContent();
                ...
                result.writeTo(out);
                String cpResponse = new String(out.toByteArray());
                logger.info("Callws: " + url.trim() + "=>" + cpResponse);
                convertSOAPBodyToProperties2(result.getSOAPBody(),pro);
                connection.close();
            }
        } catch (SOAPException ex) {
            logger.error("Can not connect to cp, timeout (ms): " + timeout);
            logger.error(" url: " + url);
            logger.error(" xml: " + xml);
            logger.error(ex);
        } catch (Exception ex) {
            logger.error("Can not connect to cp, timeout (ms): " + timeout , ex);
            logger.error(" url: " + url);
            logger.error(" xml: " + xml);
        } 
...
//                byte[] buffer = new byte[192];
    ...
    /**
     * timeout is milisecond
     */
    public String sendXml(String url, String xml, Map<String,String> pro, final int timeout) throws SOAPException, IOException{
...
//                String cpResponse = result.getSOAPPart().getTextContent();
     ...
     * timeout is milisecond
     */
    public String sendXml(String url, String xml, Map<String,String> pro, final int timeout, List<KeyValue<String, String>> keyValues,
            String nodeTag, String nameTag, String valueTag) throws SOAPException, IOException{
        String cpResponse = null;
        SOAPConnection connection = null;
        try {
            connection = SOAPConnectionFactory.newInstance().createConnection();
...
//                String cpResponse = result.getSOAPPart().getTextContent();
    ...
    private void convertSOAPBodyToProperties2(Node element, Map<String,String> pro
            ...
            String nodeTag, String nameTag, String valueTag) {
        NodeList nodeList = element.getChildNodes();
        int length = nodeList.getLength();
        for (int j = 0; j < length; j++) {
            Node node = nodeList.item(j);
