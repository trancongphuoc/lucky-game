<%@page import="com.viettel.captcha.ImageUtil"%>
<%@page import="com.viettel.captcha.StringUtil"%>
<%@page import="com.vas.util.*" %>
...
<%  
    ...
    int [] x = {4,5,6};// 4,5,6 là số captcha muốn hiện ngẫu nhiên.
    Random r = new Random(); 
    int indramdom = x[r.nextInt(x.length)];
    try {
        strImage = StringUtil.getRandString(indramdom);
        response.setContentType("image/jpeg");
        os = response.getOutputStream();
        /*
        ImageUtil.writeJPEG
                (ImageUtil.makeImage(strImage,
                        "Rockwell Extra Bold",
                        90),
                        os);
                        */
        strPath = getServletConfig().getServletContext().getRealPath("/");
        strPath += "template/images/capcharBg2.jpg";
        ImageUtil.writeJPEG(ImageUtil.drawTextOnImages(strPath,strImage), os);
        session.setAttribute("CAPTCHA",ParamsVT.enc.encode(strImage));
        //Params.enc.encode(strImage));
    } catch (Exception ex){
        System.out.println("capcharError-->" + ex.getMessage());
        ex.printStackTrace();
    } finally {
        os.flush();
        os.close();
